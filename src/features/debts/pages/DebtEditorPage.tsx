  ...
  const save = useMutation({
    mutationFn: (values: FormValues) => {
      const normalizedPayments = values.payments.map((payment) => {
        ...
        const interestAmount = Number(payment.interestAmount || 0)
        return { ...payment, principalAmount, interestAmount, amountPaid: principalAmount + interestAmount, paymentMethod: selected?.name ?? payment.paymentMethod, paymentMethodId: payment.paymentMethodId || null, accountId: payment.accountId || null }
      })
      const payload = { ...values, totalAmount: Number(values.totalAmount), receivedAmount: values.disbursesFunds ? Number(values.receivedAmount) : null, receivedDate: values.disbursesFunds ? values.receivedDate : null, depositAccountId: values.disbursesFunds ? values.depositAccountId : null, numberOfPayments: Number(values.numberOfPayments), payments: normalizedPayments, paymentsMade: normalizedPayments.filter((p) => p.isPaid).length, isFullyPaid: normalizedPayments.filter((p) => p.isPaid).reduce((sum, p) => sum + p.principalAmount, 0) >= Number(values.totalAmount) }
  ...
  return <FormPageLayout title={isUpdate ? 'Editar deuda' : 'Registro de deuda'} description={`${isUpdate ? 'Actualiza' : 'Captura'} la información de la deuda y su plan de pagos.`}
    ...
    {record.isLoading ? <div className="grid min-h-64 place-items-center"><CircularProgress /></div> : <Paper component="form" onSubmit={handleSubmit((values) => save.mutate(values))} elevation={0} sx={{ border: '1px solid', borderColor: 'divider', p: { xs: 2, sm: 3 } }}>
      ...
      <div className="grid grid-cols-1 gap-x-5 md:grid-cols-2">
        <TextField {...register('creditorName', { required: 'El acreedor es obligatorio.', maxLength: 256 })} error={Boolean(errors.creditorName)} helperText={errors.creditorName?.message ?? ' '} label={<FieldLabel required>Acreedor</FieldLabel>} />
        <TextField {...register('totalAmount', { required: true, min: .01, valueAsNumber: true })} error={Boolean(errors.totalAmount)} helperText={errors.totalAmount ? 'El monto debe ser mayor a cero.' : ' '} label={<FieldLabel required>Monto total</FieldLabel>} type="number" />
        <TextField {...register('currency', { required: true })} disabled={currencies.isLoading}
          helperText={currencies.error ? 'No fue posible cargar las monedas.' : ' '}
        ...
        <TextField {...register('dueDate', { required: true })} helperText=" " label={<FieldLabel required>Fecha de vencimiento</FieldLabel>} type="date" />
        <TextField {...register('numberOfPayments', { required: true, min: 1, valueAsNumber: true })} helperText=" " label={<FieldLabel required>Número de pagos</FieldLabel>} type="number" />
        ...
        {watch('disbursesFunds') && <>
          <TextField {...register('receivedAmount', { required: true, min: .01, valueAsNumber: true })} helperText=" " label={<FieldLabel required>Monto recibido</FieldLabel>} type="number" />
          <TextField {...register('receivedDate', { required: true })} helperText=" " label={<FieldLabel required>Fecha de depósito</FieldLabel>} type="date" />
          <TextField {...register('depositAccountId', { required: true })} className="md:col-span-2" helperText="Aumenta el saldo disponible, pero no los ingresos ordinarios." label={<FieldLabel required>Cuenta que recibió el dinero</FieldLabel>} select>
      ...
      <div className="grid gap-3">{payments.fields.map((payment, index) => <Paper key={payment.id} variant="outlined" sx={{ p: 2 }}><div className="grid grid-cols-1 gap-x-4 md:grid-cols-2 xl:grid-cols-4">
        <TextField {...register(`payments.${index}.paymentDate`, { required: true })} helperText=" " label={<FieldLabel required>Fecha</FieldLabel>} type="date" />
        <TextField {...register(`payments.${index}.principalAmount`, { required: true, min: .01, valueAsNumber: true })} helperText="Reduce el saldo de la deuda." label={<FieldLabel required>Capital</FieldLabel>} type="number" />
        <TextField {...register(`payments.${index}.interestAmount`, { min: 0, valueAsNumber: true })} helperText="Costo financiero." label="Intereses y comisiones" type="number" />
        <TextField value={Number(watch(`payments.${index}.principalAmount`) || 0) + Number(watch(`payments.${index}.interestAmount`) || 0)} helperText="Salida total de efectivo." label="Total del pago" slotProps={{ htmlInput: { readOnly: true } }} />
        <TextField {...register(`payments.${index}.paymentMethodId`, { required: true })} helperText={methods.data?.some((method) => method.active) ? ' ' : 'Primero registra un método de pago en Catálogos.'} label={<FieldLabel required>Método de pago</FieldLabel>} select>
          {methods.data?.filter((method) => method.active).map((method) => <MenuItem key={method.id} value={method.id}>{method.name}</MenuItem>)}
        </TextField>
        <TextField {...register(`payments.${index}.accountId`, { required: true })} helperText="Cuenta desde donde saldrá el pago." label={<FieldLabel required>Cuenta de origen</FieldLabel>} select>
          {accounts.data?.filter((account) => account.active).map((account) => <MenuItem key={account.id} value={account.id}>{account.name} · {account.currency}</MenuItem>)}
        </TextField>
