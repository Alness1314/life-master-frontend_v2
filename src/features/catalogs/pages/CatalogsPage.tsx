import { Alert, Card, CardActionArea, CardContent, CircularProgress, Typography } from '@mui/material'
    ...
    >
      {isLoading && (
        <div className="grid min-h-52 place-items-center">
          <CircularProgress aria-label="Cargando módulos" />
        </div>
      )}

      {error && (
        <Alert severity="error">
          No fue posible cargar los módulos de catálogos.
        </Alert>
      )}

      {!isLoading && !error && modules.length === 0 && (
        <Alert severity="info">No hay módulos disponibles para este perfil.</Alert>
      )}
